...
def extract_audio_segment(video_path, start_t=None, end_t=None, sr=SAMPLE_RATE):
    ...
    try:
        ...
        print(
            f"Audio extraction error: {video_path}, segment [{start_t},{end_t}]: {e}",
            flush=True,
        )
...
def concat_fake_segments(video_path, fake_periods):
    ...
    for start_s, end_s in fake_periods:
        ...
        seg_audio, _ = extract_audio_segment(
            video_path, start_t=start_s, end_t=end_s, sr=sr
        )
        if len(seg_audio) > 0:
...
def save_windows_as_images(
    frames, fake_frames, mel_spectrogram, output_dir, base_name, video_path
):
    """
    ...
    mel_img = (255 * (mel_spectrogram - mel_min) / (mel_max - mel_min + 1e-8)).astype(
        np.uint8
    )
    ...
    for i in range(0, N_EXTRACT):
        ...
        fake_indices = [
            ...
            if any(
                fake_start <= idx <= fake_end for fake_start, fake_end in fake_frames
            )
        ...
        sub_mel_resized = cv2.resize(
            sub_mel, (OUTPUT_SIZE[0] * WINDOW_LEN, OUTPUT_SIZE[1])
        )
...
def process_entry(entry):
    ...
    save_windows_as_images(
        frames, fake_frames, mel_spectrogram, out_dir, base_name, video_path
    )
